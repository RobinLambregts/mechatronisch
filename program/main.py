...
class PIDRegelaar:
    ...
    def bereken(self, doel, huidige_waarde, dt):
        fout = doel - huidige_waarde
        self.integraal += fout * dt
        afgeleide = (fout - self.vorige_fout) / dt if dt > 0 else 0
        output = (self.kp * fout) + (self.ki * self.integraal) + (self.kd * afgeleide)
        self.vorige_fout = fout
        return output
...
def compensate(doel3):
    ...
    fout = doel3 - (p1 + p2)
    while fout < 0:
        if p2 <= motor2.min_positie:
            break
        p2 -= 0.5
        fout += 0.5
    while fout > 3:
        if p2 >= motor2.max_positie:
            break
        p2 += 0.5
        fout -= 0.5  
    p1_final = p1
...
def automatische_loop():
    ...
    print(f"\n[START] Automatisch proces gestart. M3 beweegt via PID naar {motor3.max_positie}...")
    pid = PIDRegelaar(kp=0.5, ki=0.01, kd=0.02)
    laatste_tijd = time.time()
    while motor3_pos < (motor3.max_positie - 0.01) and automatisch_actief:
        nu = time.time()
        dt = nu - laatste_tijd
        laatste_tijd = nu
        if dt <= 0 or dt > 0.5:
        ...
        berekende_stap = pid.bereken(motor3.max_positie, motor3_pos, dt)
        stap = clamp(berekende_stap, MIN_STAP, MAX_STAP)
        motor3_pos = clamp(motor3_pos + stap, motor3.min_positie, motor3.max_positie)
        beweeg_sync(motor3_pos)
        time.sleep(0.005)
    if motor3_pos >= (motor3.max_positie - 0.01):
        motor3_pos = motor3.max_positie
        beweeg_sync(motor3_pos)
