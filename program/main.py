...
def main():
    ...
    def move_function_2(current_position, doelpositie):
        hoek_start = math.cos(current_position / 8.0)
        hoek_doel = math.cos(doelpositie / 8.0)
        verschil = hoek_doel - hoek_start
        return (verschil / (math.pi / 2) * 460)
    ...
    motor1 = Motor(dir_pin=17, pulse_pin=22, step_size=0.001, move_function=move_function_1, max_positie=3, startpositie=0, min_positie=-2)
    ...
    motor3 = Motor(dir_pin=23, pulse_pin=24, step_size=0.05, move_function=move_function_3, max_positie=9, startpositie=-1, min_positie=-1)

    motor3.beweeg_naar(8)
