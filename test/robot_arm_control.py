...
# PI Parameters
KP = 20    # Proportionele factor
KI = 0.9   # Integraal-factor: bouwt kracht op als het doel niet bereikt wordt
MAX_I = 15 # Anti-windup: de maximale bijdrage van de I-term aan de duty cycle

# ----- NIEUW: afremzone & settling timer -----
# Binnen REMZONE graden van het doel wordt de duty cycle extra afgeknepen
# zodat de motor merkbaar trager nadert.
REMZONE = 8.0       # graden — begin van afremmen (pas aan naar smaak)
REMFACTOR = 0.45    # schaalfactor op MAX_DC binnen de remzone (0–1)
                    # 0.45 → max 45 % van MAX_DC als je <REMZONE° van het doel zit

# Hoelang de hoek onafgebroken binnen TOLERANTIE moet blijven voor de motor stopt
SETTLE_TIME = 0.4   # seconden (verander naar 0.3–0.6 naar wens)
# ---------------------------------------------
...
# Staat voor de PI regelaar + settling timer
pi_staat = {
    2: {'integraal': 0.0, 'vorige_tijd': None, 'binnen_tol_sinds': None},
    3: {'integraal': 0.0, 'vorige_tijd': None, 'binnen_tol_sinds': None},
}
doel_lock = threading.Lock()

def bereken_pi_dc(m_id, fout):
    """
    PI-regelaar met afremzone vlak bij het doel.

    Normale zone (|fout| >= REMZONE):
        duty cycle loopt lineair op met de fout, max MAX_DC.

    Afremzone (|fout| < REMZONE):
        hetzelfde PI-signaal, maar het plafond zakt naar
        REMFACTOR * MAX_DC zodat de motor merkbaar trager nadert.
    """
    nu = time.time()
    staat = pi_staat[m_id]
    ...
    # ---- NIEUW: maximale duty cycle daalt in de remzone ----
    abs_fout = abs(fout)
    if abs_fout < REMZONE:
        # Lineair afschalen: op REMZONE° → vol plafond, op 0° → REMFACTOR * plafond
        schaal = REMFACTOR + (1.0 - REMFACTOR) * (abs_fout / REMZONE)
        effectief_max = MAX_DC * schaal
    else:
        effectief_max = MAX_DC
    # --------------------------------------------------------

    base_min = MIN_DC[m_id]
    dc = int(
        base_min +
        (effectief_max - base_min) *
        min(output_abs / 15.0, 1.0)
    )

    return max(base_min, min(int(effectief_max), dc)), richting
    ...
    while motor_systeem_actief:
        ...
        for imu_addr, m_id in IMU_MOTOR_MAP.items():
            if m_id not in actieve_doelen:
                ...
                pi_staat[m_id]['binnen_tol_sinds'] = None  # reset timer
                continue
            ...
            if abs(fout) <= TOLERANTIE:
                # ---- NIEUW: settling timer ----
                if pi_staat[m_id]['binnen_tol_sinds'] is None:
                    # Net de tolerantieband binnengekomen
                    pi_staat[m_id]['binnen_tol_sinds'] = nu

                verstreken = nu - pi_staat[m_id]['binnen_tol_sinds']

                if verstreken >= SETTLE_TIME:
                    # Lang genoeg stabiel → motor stoppen
                    pwm_motoren[m_id].ChangeDutyCycle(0)
                    pi_staat[m_id]['integraal'] = 0.0
                    pi_staat[m_id]['binnen_tol_sinds'] = None
                    with doel_lock:
                        motor_doel[m_id]['actief'] = False
                else:
                    # Nog wachten: motor zachtjes laten lopen met gereduceerde dc
                    dc, richting = bereken_pi_dc(m_id, fout)
                    GPIO.output(config[m_id]['dir'], richting)
                    pwm_motoren[m_id].ChangeDutyCycle(dc)
                # --------------------------------
            else:
                # Buiten tolerantie: timer resetten en normaal rijden
                pi_staat[m_id]['binnen_tol_sinds'] = None
                dc, richting = bereken_pi_dc(m_id, fout)
                GPIO.output(config[m_id]['dir'], richting)
                pwm_motoren[m_id].ChangeDutyCycle(dc)
        ...
        try:
            ...
            with doel_lock:
                for m_id, doel in doelen_parsed.items():
                    ...
                    # Reset settling timer bij nieuw doel
                    pi_staat[m_id]['binnen_tol_sinds'] = None
...
try:
    while True:
        ...
        if doel_m2['actief']:
            ...
            # Toon settling-status in de UI
            settle2 = pi_staat[2]['binnen_tol_sinds']
            settle_txt2 = f"  settle: {round(time.time()-settle2, 2)}s" if settle2 else ""
            tekst(frame, f"Doel M2: {doel_m2['doel']}°  fout: {fout2}°{settle_txt2}", 135, (0, 255, 100))
        else:
            tekst(frame, "Doel M2: inactief", 135, (150, 150, 150))

        if doel_m3['actief']:
            fout3 = round(doel_m3['doel'] - (hoek2 or 0), 1)
            settle3 = pi_staat[3]['binnen_tol_sinds']
            settle_txt3 = f"  settle: {round(time.time()-settle3, 2)}s" if settle3 else ""
            tekst(frame, f"Doel M3: {doel_m3['doel']}°  fout: {fout3}°{settle_txt3}", 165, (0, 255, 100))
        else:
        ...
        tekst(frame, f"Remzone:{REMZONE}° factor:{REMFACTOR} | Settle:{SETTLE_TIME}s", 270, (180, 100, 255))
        tekst(frame, "ESC=afsluiten | Terminal: doelhoek/'hoek'/'calibrate'", 305, (180, 180, 180))
